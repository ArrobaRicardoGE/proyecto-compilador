    ...
    int sintacticAnalyzer::analyzeToBracket(gm::tokenMatrix &tokens, std::vector<std::vector<int>>& ids,
        ...
        for(i=index; i < ids.size(); i++)
        {
            ...
            if(ids[i][0] == VARIABLE && ids[i][1] == ASIGNAR && ids[i][2] != READ)
            {
                ...
                std::unique_ptr<expresionMatematica> evaluable(new expresionMatematica(std::move(tokensExpresion)));
                ...
                std::unique_ptr<read> lectura(new read(std::move(vacio)));
                ...
                if(ids[i][3] == READ)
                {
                    std::unique_ptr<read> lectura(new read(std::move(tokensExpresion)));
            ...
            else if(ids[i][0] == WHILE && ids[i][ids[i].size()-1] == '{')
            {
                ...
                std::unique_ptr<expresionMatematica> evaluable(new expresionMatematica(tokensExpresion));
            ...
            else if(ids[i][0] == FOR && ids[i][ids[i].size()-1] == '{')
            {
                ...
                if(j != ids[i].size() && ids[i][1] == VARIABLE && ids[i][2] == ASIGNAR && j>3)
                {
                    ...
                    std::unique_ptr<expresionMatematica> exp1(new expresionMatematica(std::move(tokensExpresion1)));
                    std::unique_ptr<expresionMatematica> exp2(new expresionMatematica(std::move(tokensExpresion2)));
            ...
            {
                std::vector<std::string> tokensExpresion;
                copia(i, i, ids[i].size()-1, tokens, tokensExpresion);
                std::unique_ptr<expresionMatematica> exp(new expresionMatematica(std::move(tokensExpresion)));
